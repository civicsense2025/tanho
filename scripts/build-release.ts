/**
 * build-release.ts — builds, signs, and publishes a platform release to the OYS Hub.
 *
 * This is the vendor-side (template repo) half of the update-delivery contract. The
 * customer-side halves are `@own-your-site/cli` (verifies + 3-way merges) and the hub
 ...
 *   3. Bakes `.own-your-site/version.json` = { version, baseCommit, releasedAt } INTO
 ...
 *   HUB_URL=https://hub.own-your-site.com \
 *   HUB_ADMIN_TOKEN=... \
 *   ED25519_PRIVATE_KEY="$(cat oys-priv.pem)" \
 *   npx tsx scripts/build-release.ts --version 0.2.0
 *
 * ENV
 *   HUB_URL              Hub base URL (default https://hub.own-your-site.com).
 ...
 * .own-your-site/version.json into it. Returns the final ZIP bytes.
 *
...
function buildZip(version: string, baseCommit: string, releasedAt: string): Buffer {
  const work = mkdtempSync(join(tmpdir(), "oys-release-"));
  const zipPath = join(work, `own-your-site-${version}.zip`);
  try {
    ...
    // 2. Bake .own-your-site/version.json into the SAME zip so it's covered by the
    //    signature. `zip -j` would junk paths; we need the nested path, so stage it.
    const stageDir = join(work, "stage");
    mkdirSync(join(stageDir, ".own-your-site"), { recursive: true });
    const versionJson = JSON.stringify({ version, baseCommit, releasedAt }, null, 2) + "\n";
    writeFileSync(join(stageDir, ".own-your-site", "version.json"), versionJson);
    // Add to the existing archive, preserving the .own-your-site/ path.
    execFileSync("zip", ["-q", zipPath, join(".own-your-site", "version.json")], { cwd: stageDir });
...
async function main(): Promise<void> {
  ...
  const assetFilename = `own-your-site-${version}.zip`;
  ...
  if (args.unsigned) {
    console.warn("  ⚠ --unsigned: skipping Ed25519 signature (the CLI will REFUSE this release unless OYS_DEV_UNSAFE=1).");
  } else {
