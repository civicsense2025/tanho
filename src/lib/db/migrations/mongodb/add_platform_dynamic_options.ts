...
const CORRECTED_GUIDE_FIELDS = [
  ...
  { key: "sourcePlatform", label: "Source Platform", kind: "select", dynamicOptions: "platforms" },
...
/** See migrations/libsql/add_platform_dynamic_options.ts for the full rationale. */
export const addPlatformDynamicOptions: MongoMigration = {
  name: "add_platform_dynamic_options",
  async run(db) {
    await db.collection("content_types").updateOne(
      { slug: "guide", isBuiltIn: 1, fields: { $not: /dynamicOptions/ } },
