...
export const seedBuiltinTypes: PostgresMigration = {
  ...
  sql: BUILTIN_TYPES.map(
    (t) =>
      `INSERT INTO content_types (id, slug, name, icon, fields, is_built_in, sort_order, seo_title_template, seo_description_template, created_at, updated_at) VALUES (gen_random_uuid(), '${t.slug}', '${t.name}', '${t.icon}', '${t.fields.replace(/'/g, "''")}', 1, ${t.sortOrder}, NULL, NULL, NOW(), NOW()) ON CONFLICT (slug) DO NOTHING`
  ).join(";\n"),
};
