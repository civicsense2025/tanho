 ...
 * (pages + entries + /resources + /shop + every published type's index URL),
 * one "shop" section per chunk of active products, and one or more "type"
 * sections per table-backed content type, each holding up to SITEMAP_CHUNK_SIZE
 * row URLs. The route handlers app/sitemap/[id]/route.ts (each section) and
 * app/sitemap.xml/route.ts (the index) both read this same section list, so
 * the index and the children never disagree. This module is the single source
 * of what those ids mean.
 */
...
/** Make a stored media URL absolute for sitemap/image-tag output. */
function absoluteMediaUrl(base: string, url: string | null | undefined): string | null {
  if (!url) return null;
  if (/^https?:\/\//i.test(url)) return url;
  return `${base}${url.startsWith("/") ? url : `/${url}`}`;
}

/** Resolve a batch of media ids to absolute public URLs. */
async function resolveMediaUrls(mediaIds: Array<string | null | undefined>): Promise<Map<string, string>> {
  ...
  for (const row of rows) {
    // mediaPublicUrl returns a storage adapter URL; make it absolute for the sitemap.
    const url = await mediaPublicUrl(row.id);
...
export async function listSitemapSections(
  ...
  const activeProductCount = await db.$count(products, eq(products.status, "active"));
  if (activeProductCount > 0) {
    const productChunks = Math.ceil(activeProductCount / chunkSize);
    for (let chunk = 0; chunk < productChunks; chunk++) {
      sections.push({ id: id++, kind: "shop", chunk });
    }
  }
...
/** The core section: pages + entries + /resources + /shop + every published type's index URL. */
async function buildCoreSection(base: string): Promise<SitemapEntry[]> {
  const contentTypes = await getContentTypesSettings();
  ...
  const pageRows = await db
    .select({
      ...
      ogImageMediaId: pages.ogImageMediaId,
    })
    .from(pages)
    .where(eq(pages.status, "published"));

  const pageMediaIds = pageRows
    .filter((p) => !p.noIndex)
    .map((p) => p.ogImageMediaId)
    .filter((id): id is string => Boolean(id));
  const pageImageUrlById = await resolveMediaUrls(pageMediaIds);
  ...
  const items: SitemapEntry[] = pageRows
    .filter((p) => !p.noIndex)
    .map((p) => {
      const imageUrl = absoluteMediaUrl(base, pageImageUrlById.get(p.ogImageMediaId ?? ""));
      ...
      if (imageUrl) {
        entry.images = [{ loc: imageUrl, title: p.title, caption: p.title }];
      }
      return entry;
    });
  ...
  // Shop index and collection pages live in the core section (so /shop is
  // discoverable from the index even when products are chunked into shop sections).
  const collectionRows = await db.query.collections.findMany();
  const visibleCollections = collectionRows.filter((c) => c.visible);
  const collectionMediaIds = visibleCollections.map((c) => c.coverMediaId).filter((id): id is string => Boolean(id));
  const collectionImageUrlById = await resolveMediaUrls(collectionMediaIds);

  // The /shop index is part of the storefront; include it in the core section so
  // the index points to the main catalog landing page.
  items.push({
    url: abs("/shop"),
  ...
  for (const c of visibleCollections) {
    const imageUrl = absoluteMediaUrl(base, collectionImageUrlById.get(c.coverMediaId ?? ""));
    const entry: SitemapEntry = {
      url: abs(`/shop?collection=${encodeURIComponent(c.id)}`),
      lastModified: new Date(),
      changeFrequency: "daily",
      priority: 0.6,
    };
    if (imageUrl) {
      entry.images = [{ loc: imageUrl, title: c.name, caption: c.name }];
    }
    items.push(entry);
  }
...
/** One shop section: a chunk of active product URLs at /shop/:slug. */
async function buildShopSection(
  ...
  for (const p of productRows) {
    const images: SitemapEntry["images"] = [];
    for (const src of p.images ?? []) {
      const loc = absoluteMediaUrl(base, src);
      if (loc) images.push({ loc, title: p.name, caption: p.name });
    }
    const entry: SitemapEntry = {
      url: abs(`/shop/${p.slug}`),
      lastModified: new Date(p.updatedAt),
...
export async function buildSitemapForSection(
  ...
  if (descriptor.kind === "shop") return buildShopSection(descriptor, base, chunkSize);
